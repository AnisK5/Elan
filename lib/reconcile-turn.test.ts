...
describe("suivi d'appel — Sogessur", () => {
  ...
  it("« maj » / « pardon » / « pas à jour » reprennent le dump d'avant", () => {
    expect(isCarryForwardTurn("maj")).toBe(true);
    expect(isCarryForwardTurn("sogessur pardon (le numero que je devais appeler aujourd'hui)")).toBe(
      true,
    );
    expect(isCarryForwardTurn("c'est pas a jour")).toBe(true);
    expect(isCarryForwardTurn("la suite est pas ecrite en desc")).toBe(true);
