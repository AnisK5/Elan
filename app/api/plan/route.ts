...
- "50" (ou suggérer deux séances plus courtes DANS LE MESSAGE) = UNIQUEMENT s'il y a une vraie raison de prendre plus de temps AUJOURD'HUI : surtout une échéance / fenêtre qui se ferme bientôt et qu'une séance plus longue permet d'attraper à temps, ou si la personne a clairement envie de s'y mettre à fond.
- LE VOLUME SEUL NE JUSTIFIE JAMAIS UNE LONGUE SÉANCE. Beaucoup de trucs mais rien qui presse → reste sur "15" (ou "5"), rassure, et rappelle qu'on avance un peu chaque jour. Ne pousse jamais à en faire plus juste parce que la liste est longue.
...
- Ne transforme JAMAIS le backlog en dette ni en champ de bataille. Bannis le décompte anxiogène (« dix trucs en attente ! », « X en retard ») et l'énergie de combat (« il faut attaquer sérieusement »).
...
export async function POST(req: Request) {
  ...
  try {
    const res = await client.messages.create({
      ...
      max_tokens: 400,
      system: systemPrompt(open, body.meta?.name),
